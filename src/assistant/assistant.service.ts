...
export class AssistantService {
  ...
  constructor(
    private readonly llm: AssistantLlmProvider,
    private readonly tools: AssistantToolsService,
    private readonly config: ConfigService,
  ) {}

  async chat(request: AssistantChatDto): Promise<AssistantResponse> {
    ...
    try {
      ...
      return {
        ...
        type: 'ERROR',
        assistantMessage:
          'I could not process that safely right now. You can still use the report button or ask for help.',
        speakableMessage: 'I could not process that safely right now.',
      };
    }
  }

  async confirmAction(dto: ConfirmAssistantActionDto): Promise<AssistantResponse> {
    ...
    try {
      ...
      return {
        ...
        assistantMessage:
          `${this.humanHazard(dto.payload.hazardType)} report submitted successfully. Other drivers nearby will be warned.`,
      ...
      return {
        ...
        type: 'ERROR',
        assistantMessage:
          'I could not submit that report. Please use the report screen or try again when the connection is stable.',
    ...
    switch (intent.intent) {
      case 'HELP':
        return {
          ...
          type: 'GENERAL_HELP',
          assistantMessage:
            'I can explain routes, check nearby hazards, draft pothole, accident, or flood reports, and give short safety guidance. Use voice when driving.',
          speakableMessage:
            'I can explain routes, check nearby hazards, draft reports, and give safety guidance.',
  ...
  private async nearbyHazards(request: AssistantChatDto): Promise<AssistantResponse> {
    if (!request.location) {
      return this.clarify('Share your location first so I can check nearby hazards.');
    }

    const hazards = await this.tools.findNearbyHazards(
      request.location.lat,
      request.location.lng,
      request.radiusMeters ?? 1500,
    );
    if (hazards.length === 0) {
      return {
        success: true,
        type: 'NEARBY_HAZARDS',
        assistantMessage:
          'I do not see active hazards near your current location. Keep your attention on the road.',
    ...
    return {
      ...
      type: 'NEARBY_HAZARDS',
      assistantMessage:
        `I found ${hazards.length} active hazard${hazards.length === 1 ? '' : 's'} nearby: ` +
        top.map((h: any) => this.humanHazard(h.type)).join(', ') +
        '. Slow down and stay alert.',
      speakableMessage:
        `I found ${hazards.length} active nearby hazard${hazards.length === 1 ? '' : 's'}. Slow down and stay alert.`,
      data: { hazards },
    };
  }
  ...
  private hazardDraft(
    ...
    const draft = this.tools.createHazardReportDraft(
      hazardType,
      request.location.lat,
      request.location.lng,
      `Reported by AI assistant: ${this.humanHazard(hazardType)}`,
    );
    const requireConfirmation =
      this.config.get<string>('ASSISTANT_REQUIRE_CONFIRMATION', 'true') !== 'false';

    if (!requireConfirmation) {
      // Confirmation remains the safer default. This branch is intentionally unused
      // unless deployment policy explicitly enables trusted one-tap reporting.
    }
    ...
    return {
      ...
      assistantMessage:
        `I detected a ${this.humanHazard(hazardType).toLowerCase()} report near your current location. Please confirm before I submit it.`,
      speakableMessage:
        `${this.humanHazard(hazardType)} report ready. Please confirm before I submit it.`,
      suggestedActions: [
        {
  ...
  private routeExplanation(request: AssistantChatDto): AssistantResponse {
  ...
  private async navigationDestination(
    request: AssistantChatDto,
    intent: LlmToolIntent,
  ): Promise<AssistantResponse> {
    if (!request.location) {
    ...
    const route = await this.tools.calculateRoute(
    ...
    const distanceMeters =
      route?.id === 'fallback_route' || routeDistanceMeters <= 0
        ? straightLineMeters
        : routeDistanceMeters;
  ...
  private async safetySummary(request: AssistantChatDto): Promise<AssistantResponse> {
    const summary = await this.tools.getSafetySummary(
      request.location?.lat,
      request.location?.lng,
      request.radiusMeters ?? 1500,
    );
  ...
  private classifyFallback(message: string): LlmToolIntent {
  ...
  private shouldUseFallback(
    llmIntent: LlmToolIntent | null,
    fallbackIntent: LlmToolIntent,
  ) {
    if (!llmIntent) return true;
    if (llmIntent.intent === 'CLARIFY' && fallbackIntent.intent !== 'CLARIFY') {
      return true;
    }
    if (
      llmIntent.intent === 'NAVIGATION_DESTINATION' &&
  ...
  private isDistanceQuestion(message: string) {
    const text = message.toLowerCase();
  ...
  private toReportType(value?: string, message = ''): ReportType {
    const text = `${value ?? ''} ${message}`.toLowerCase();
    if (text.includes('pothole')) return ReportType.POTHOLE;
    if (text.includes('flood')) return ReportType.FLOOD;
    if (text.includes('accident')) return ReportType.ACCIDENT;
    if (text.includes('crash')) return ReportType.CRASH;
    if (text.includes('closure') || text.includes('blocked')) return ReportType.CLOSURE;
    if (text.includes('traffic')) return ReportType.TRAFFIC;
    return ReportType.HAZARD;
  }

  private humanHazard(type: string) {
    return type
      .replace(/_/g, ' ')
      .replace(/\b\w/g, (letter) => letter.toUpperCase());
  }

  private clarify(message: string): AssistantResponse {
    return {
      success: true,
      type: 'CLARIFICATION',
  ...
  private quickActions() {
    return [
      { id: 'qa_help', label: 'Help', type: 'QUICK_REPLY' as const, payload: { message: 'What can you do?' } },
      { id: 'qa_hazards', label: 'Nearby hazards', type: 'QUICK_REPLY' as const, payload: { message: 'Any hazards near me?' } },
      { id: 'qa_pothole', label: 'Report pothole', type: 'QUICK_REPLY' as const, payload: { message: 'Report pothole ahead' } },
    ];
  }
}
